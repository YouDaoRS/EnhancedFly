...
EnhancedFly 是 Java 17 编写的 Bukkit/Spigot API 1.20.1 飞行插件，主要运行目标是 Mohist 1.20.1。修改应优先保证玩家飞行余额、经济购买和持久化数据安全，同时不干扰创造模式、旁观模式或其他插件提供的飞行。
...
- 尚未完成 Mohist 1.20.1、真实 Vault 经济提供者及真实 MySQL 的集成测试。
- Vault 扣款与插件数据库/限购文件无法构成跨系统原子事务；进程强杀或断电仍可能发生扣款与发货短暂不一致。
- 当前存储模型面向单服务器，不提供多台服务器共享玩家余额时的分布式并发控制。
- 不要声称插件支持所有 Spigot 分支、模组组合或其他飞行插件，除非已有对应实测证据。
