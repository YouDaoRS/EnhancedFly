...
EnhancedFly 是面向 Minecraft 1.20.1 服务端的飞行管理插件，支持临时与永久飞行、Vault 经济商店、多世界限制、飞行统计、成就、SQLite、MySQL 和本地 YAML 存储。
...
- Java 17
- Spigot API 兼容的 Minecraft 1.20.1 服务端；项目主要面向 Mohist 1.20.1
- Vault 及一个经济插件（仅商店经济功能需要）

Mohist、Vault 和真实 MySQL 环境仍需在测试服务器中完成集成验证。当前自动化测试使用真实 SQLite，Bukkit 玩家和调度行为使用模拟对象。

## 安装

1. 从 [Releases](https://github.com/YouDaoRS/EnhancedFly/releases) 下载 `EnhancedFly-2.1.1.jar`。
...
```text
build/libs/EnhancedFly-2.1.1.jar
```

`verifyPluginJar` 会从打包后的 JAR 加载 MySQL Driver、HikariCP 和 SQLite Driver，并使用 SQLite 原生驱动执行一次实际读写。
...
面向后续 Codex 或其他开发会话的项目约束与验证流程见 [`AGENTS.md`](AGENTS.md)。2.1.1 的详细修复记录见 [`FIXES-2.1.1.md`](FIXES-2.1.1.md)。
